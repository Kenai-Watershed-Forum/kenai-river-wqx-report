...
total_samples_collected_2021 <- read.csv('other/output/aqwms_formatted_results/2021_kwf_baseline_results_aqwms.csv') %>%
  clean_names() %>%
  group_by(result_analytical_method_id,activity_start_date) %>%
  count() %>%
...
planned_samples_2021_summary <- planned_samples_2021 %>%
  group_by(analysis,expected_results,activity_start_date,sample_type) %>%
...
5.  Were field duplicates, blanks, and/or other QC samples collected as planned? ***No; one analysis not performed in Spring 2021***

```{r planned-dups-blanks, echo = F}
...
# note total P and total NO3/NO2 from same bottle
...
planned_blanks_dups_2021 <- planned_samples_2021 %>%
  select(site,activity_start_date,analysis,expected_results,sample_type) %>%
  ...
  rename(monitoring_location_id = site,
         activity_type = sample_type,
         result_analytical_method_id = analysis) %>%
  mutate(n_planned = 1)
  
  
# expected results per analysis is not the sample measure as count... how to rectify
(z <- left_join(planned_blanks_dups_2021,actual_blanks_dups_2021))

# missing 1 200.7 from spring .. why?! check sheet. answewr: becuase the analysis was not requested on the CoC


```

Table \@ref(tab:planned-dups-blanks) summarizes the intended vs actual analyses performed for field blanks and field duplicates in 2021. Analyses were executed as planned, with the exception of the field replicate for total metals (EPA Analytical Method 200.7 for the KR RM 31 site (Morgan's Landing site) on 2021-05-11. This sample was not analysed for total metals due to a transcription error on the chain of custody form.
