...
Prior to analysis and interpretation of water quality data, we will ensure that all data that meets QA/QC standards outlined in the project Quality Assurance Project Plan (QAPP) is accessible in the appropriate repository. Water quality data for this project is ultimately destined for the EPA Water Quality Exchange (EPA WQX), formerly EPA STORET. Section B10 of the 2020 QAPP describes data management details and responsible parties for each step of the data pipeline from observation to repository. The 2021 data review process is published here as an appendix as an example of the process applied annually to each year's data.

### 2021 Water Quality Data

Water quality data generated from the 2021 Kenai River Baseline Water Quality Monitoring (KRBWQM) program was submitted to the Soldotna office of the Alaska Department of Environmental Conservation (ADEC) in April 2022 using the project-specific spreadsheet provided by ADEC. The template is intended to prepare data for submission to the [Ambient Water Quality Data Management System](https://www.awqms.com/), the service that ADEC uses to manage water quality data.
...
## correct missing collection times for ALS Spring 2021
#### Working here 3/18/2022 ::: need to get missing collection times aplied eto spring 2021 ALS data. use generate/match unique values approach

# also rec_time should be same for all of each of the 2021 dates? (spring: als received 5/14/21)

# i think this is all good for now actually. check back later.
...
# assign new schema in later steps towards final
# (3/18/2022 - WHAT DOES THIS MEAN?)
...
dat %<>%
  mutate(sample_condition = case_when(
    ...
    grepl("DUP",sample) ~ "DUP",
    grepl("Dup",sample) ~ "DUP")) %>%
  # remove "DUP" designation from sample column
  mutate(sample = str_replace(sample, "DUP|Dup", "")) 

  
# remove from "sample" names the text containing the suffixes Diss/Dis (Dissolved metals sample) since we only site info in this column. (Solution for this step was found at https://stackoverflow.com/questions/29271549/replace-all-occurrences-of-a-string-in-a-data-frame)
dat %<>%  
...
sgs21_als21_qaqc_dat <- dat %>%
    # retain only non-field sample results
   filter(!sample_type %in% c("PS","SMPL","TB","DUP1"))
...
dat %<>%
   filter(sample_type %in% c("PS","SMPL","TB","DUP1","DUP"))




# remaining issue: the 2021 ALS Electronic Data Deliverables do not include sample collection time, only date. But - we have the sample collection times populated for all the SGS data. The collection time is identical for both since they're from the same sample.

# solve this issue by creating a matching table a.) creating csv of all iterations of location; b.) populate this spreadsheet with manually entered times from ALS pdf, c.) import and left_join this df back to the main dataframe, then c.) delete the old collect_time column

# WORKING HERE 2/25/22 TO RECITIFY COLLECT TIME ISSUE DESCRIBED... WAIT TO SEE IF ALS CAN PROVIDE DATA IN EDD BEFORE PURSUING !!! Might not be essential info


#z <- dat %>%
 # filter(analytical_method == "EP200.8") %>%
#  select(monitoring_location_id,collect_date,collect_time,analytical_method,sample_condition) %>%
#  distinct()

#z1 <- dat %>%
#  select(-collect_time)

# a.) create a matching table for all iterations of collect_date, collect_time, and location
#match_df_collect_time <- sgs21 %>%
#  filter(analyte == "Total Phosphorus") %>%
#  distinct(monitoring_location_name,sample_condition, collect_date, collect_time) 
#  mutate(collect_time = "") %>%
#  filter(!is.na(monitoring_location_name))

# write.csv(match_df_collect_time,"other/input/AQWMS/match_df_collect_time_spring_als21.csv", row.names = F)


# b.) left_join this df back to the main dataframe
#z <- left_join(sgs21,match_df_collect_time, by = c("collect_date","monitoring_location_id"))


# remove matching table
#rm(match_df_collect_time)
...
######## 3/21/2022 --> no qa/qc results from SWWTP TSS on record yet. contact by email to see whats available. looking for labd duplicate and/or external qc check standard (pg 34 of QAPP)
...
```{r, echo = F}
xfun::embed_file('other/output/aqwms_formatted_results/2021_kwf_baseline_results_aqwms.csv', text = "Download All 2021 Kenai River Baseline Water Quality Monitoring Results, Formatted for AQWMS")
...
Prior to consideration for elevation to the [EPA Water Quality Portal](https://www.epa.gov/waterdata/water-quality-data), all water quality data must address a series of data evaluation questions from ADEC in order to evaluate how quality assurance / quality check (QA/QC) processes are instituted. The draft Data Evaluation Checklist Template outlines these questions.
...
1.  Did the project follow the QAPP? ***Yes***. Were there any deviations from the sampling plan? ***TBD***
...
3.  Are site names, dates, and times correct and as expected? ***Yes*** Notes: In 2021 Kenai Watershed Forum used pre-printed labels on all sample bottles, reducing opportunity for transcription errors. Remaining site name transcription errors were corrected in post-season data review.

4.  Is the dataset complete and did you receive the expected number of results? ***TBD***. We acquired the expected number of results in 2021 sampling events. All sites described in the QAPP were visited twice, and all water sample collections were successfully conducted. However intrinsic water quality parameters mentioned in the 2020 QAPP were not collected in 2021. Data evaluation is currently in progress and the values below are current as of `r Sys.Date()`:

-   Enter Completeness Goal from QAPP (%): ***TBD***

-   Enter Project Completeness (%): ***TBD***


5. WORKING HERE 3/21/2022 --- REVIEW INVENTYORY FIRST!!!
