...
# move info about duplicate sample status in separate column
sgs21 %<>%
  mutate(sample_condition = str_extract(client_sample,"DUP")) %>%
  mutate(client_sample = str_replace(client_sample, "DUP", ""))
  
  
# remove from site names the text containing the suffixes "DUP" (duplicate) or Diss/Dis (Dissolved metals sample) since we only want a list of unique sites. (Solution for this step found at https://stackoverflow.com/questions/29271549/replace-all-occurrences-of-a-string-in-a-data-frame)
sgs21 %<>%  
...
# for summer samples, need to address "Trip blanks"; look @ field forms to figureout which ones went with which group ...(no name (KWF) vs others (USFWS))
