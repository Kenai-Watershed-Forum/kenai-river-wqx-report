...
The code scripts below assembles water quality data from the three analytic laboratories that partnered with Kenai Watershed Forum for this project in 2021:
...
Download Original 2021 Metals/Nutrients Lab Results (Click Link)
...
##### Fecal Coliform Lab Results (Soldotna Wastewater Treatment Plant/Taurianen Engineering)
...
swwtp_spring21 %<>%
  ...
  mutate(analysis_time_in = as_hms(time_in),
         analysis_date_in = spring21_sample_date) %>%
  select(-time_in,-time_out) %>%
  # field sample time/date
  mutate(time_sampled = as_hms(time_sampled),
         sample_date = spring21_sample_date)
...
## working here 2/11/2022
dat <- bind_rows(sgs21,swwtp_spring21)

################## ghet site name match
...
swwtp_summer21 <- read_excel("other/input/2021_wqx_data/summer_2021_wqx_data/Taurianen/Fecal_Coliform_Results_Spreadsheet.xlsx", skip = 3) %>%
  clean_names() %>%
...
Download Original 2021 Total Dissolved Solids Lab Results (Click Link)
...
z <- sgs21 %>%
  # Proceeding left to right across columns of AQWMS template
