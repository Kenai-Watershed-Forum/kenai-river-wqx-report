...
\**Note: the chain of custody documents for SGS Laboratories are integrated into the above downloadable files.*
...
Prior to submission to ADEC, all water quality data must address a series of data evaluation questions in order to evaluate how quality assurance / quality check (QA/QC) processes are instituted. The draft Data Evaluation Checklist Template (available for download below) outlines these questions:
...
# join with sample holding times
holding_times <- read.csv("other/input/AQWMS/sample_holding_times.csv") %>%
  transform(holding_time_hours = as_hms(anytime(holding_time_hours)))


# calc time from field sample to lab analysis


###download all edd files from als global and sgs engage
...
2.  [**Was the data collected representative of environmental conditions?**]{.ul} *Yes*. Notes: Study design is intended to create two single-day snapshots across a wide geographical area of the Kenai River watershed.
...
# embed for download
xfun::embed_file("other/output/field_qa_qc_data/rpd_calcs_2021.xlsx", text = "Download Summary Table Relative percent Difference Values between Project Samples and Field Duplicate Samples for 2021")
...
8.  Was instrument calibration performed according to the QAPP and instrument recommendations? ***Yes***. Were calibration logs or records kept? ***Yes***. Notes: ***Calibration records are available upon request at [hydrology\@kenaiwatershed.org](mailto:hydrology@kenaiwatershed.org){.email}***.
...
11. Is any field data rejected and why? ***Yes. As of this raw data review last updated on `r Sys.Date()`, the RPD value for Total Phosphorus on 5/11/2021 from the site "KR RM 31" is 77.03%, well above the project precision goal for this parameter of 20%.***.

##### Analytical Laboratory Reports and Results

12. Do the laboratory reports provide results for all sites and parameters? ***Yes. The laboratory reports provide results for all sites and parameters specified in the Chains of Custody. Howevere in one case, a transcription error on the chain of custody form for SGS labs in Spring 2021 resulted in missing result values for the method 200.7 analyses (Total Metals) from 2021-05-11 at RM 31 (Morgan's Landing)***.
...
17. Were preservation, hold time and temperature requirements met? ***Yes***. **Can provide calculation table to verify if necessary**. DO CALCULATIONS

18. Are there any project specific concerns (e.g. total v. dissolved, MST, etc)? ***Yes. Communication with the ADEC office in Winter 2022 indicates the need to slightly modify field practices with relation to dissolved metals. These practices include a.) filtering samples in lab rather than in the field, and b.) incorporating a field blank for dissolved metals. These practices are integrated into the project as of summer 2022 and will be refleccted in the updated QAPP***.
...
20. Were there any laboratory discrepancies, errors, data qualifiers, or QC failures (review laboratory duplicates, matrix spikes and blanks)? ***TBD, review PDFs***.
