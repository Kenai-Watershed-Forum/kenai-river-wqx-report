...
5.  Were field duplicates, blanks, and/or other QC samples collected as planned? ***Yes, with one exception, see below***
...
tbl <- left_join(planned_blanks_dups_2021,actual_blanks_dups_2021)


kable(tbl, caption = "2021 Planned vs actual QA/QC results available",
      col.names = c("Site",
                    "Date",
                    "Analysis",
                    "Sample Type",
...
# read in list to subset sites with field duplicates, then subsetoverall dataset
...
field_dups_2021 <- inner_join(dat_clean,qaqc_sites_2021)

# retain needed columns and restructure dataframe
z <- field_dups_2021 %>%
  select(monitoring_location_id,activity_start_date,activity_type,characteristic_name,result_analytical_method_id,result_value,result_qualifier) %>%
  pivot_wider(names_from = activity_type,values_from = result_value) %>%
  clean_names() %>%
  mutate(rpd_pct = abs((field_msr_obs - quality_control_field_replicate_msr_obs) / field_msr_obs)*100)
