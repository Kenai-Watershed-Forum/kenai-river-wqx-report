...
## 4.2: Completeness Measure B: "the planned number of samples divided by the useable samples with a goal of 60%."
...
# general approach: create count table of collected results in original aqwms file. flag those results that are not submitted to ADEC. compare two sets of values to get completeness measure A.
...
# need total samples collected summary too????

### Second: number of samples finalized and submitted to DEC
#### total_samples_usable_2021 <- get value once all QA/QC processes complete
...
## prepare and summarise table samples collected and tests performed
  ...
  # rename analysis types to match between planned and actual 
  ...
  mutate(activity_type = case_when(
    ...
    sample_type == "FIELD BLANK" ~ "Quality Control Sample-Field Blank",
    TRUE ~ "Field Msr/Obs")) %>%
  select(-sample_type)
...
# note: From the above table we can see that there are deviations between planned and actual results available. These deviations are attributable to two causes:
...
# once data that does not pass QC muster is flagged and removed, we will append an additional column
...
4.  [**Is the dataset complete and did you receive the expected number of results?**]{.ul} *TBD*. Notes: we acquired the expected number of results in 2021 sampling events. All sites described in the QAPP were visited twice in 2021, and all water sample collections were successfully conducted. However most intrinsic water quality parameters mentioned in the current QAPP were not collected in 2021. Data evaluation is currently in progress and the values below are current as of `r Sys.Date()`.
    ...
    -   Completeness Measure A: (Samples Collected) / (Samples Submitted to ADEC) \*100% = ***%TBD***
        ...
        -   Project goal: ***85%***
        -   Calculated project completeness Measure A: TBD

    -   Completeness Measure B: (Planned Number of Samples) / (Useable Samples) \* 100% = ***%TBD***

        -   Project goal: ***60%***

        -   Calculated project completeness Measure B: TBD
...
# see existing table created to address question #4, "planned_samples_2021"
...
# note bottles vs test. total P and total NO3/NO2 from same bottle
...
actual_field_blanks_2021 <- actual_blanks_dups_2021 %>%
  filter(activity_type == "Quality Control Sample-Field Blank") %>%
  count() %>%
  as.character()
...
# embed for download
xfun::embed_file("other/output/field_qa_qc_data/planned_actual_blanks_dups_2021.csv", text = "Download Summary Table of Planned vs. Actual Field Blank and Field Duplicate Samples for 2021")

```

<br>

From table \@ref(tab:planned-qaqc) we observe that data from one field duplicate sample is missing. The missing sample is for the 200.7 (total metals) analyses on 2021-05-11 ar RM 31 (Morgan's Landing). This error occurred due to a transcription error on the chain of custody form.
...
-   Field blanks:
    ...
    -   \# collected: ***`r actual_field_blanks_2021`***
...
6.  [**Are the duplicate sample(s) RPD within range described in QAPP?**]{.ul} *TBD*
...
-   View duplicate RPD calculations and a summary table in supporting excel file; including parameters, site names, dates, results, and RPD values: ***Values TBD***
    ...
    -   Note: from ADEC 2022, *"Estimated values (detected at levels lower than the practical quantitation limit (PQL)) were treated as non-detects for the purpose of this analysis. A set of paired samples was evaluated for RPD only if:*
...
###### Address which samples can be evaluated for RPD ##############
  ...
  # from DEC 2022 field report: "... a set of paired samples was evaluated for RPD only if: a.) one or both of the samples were above the PQL; AND if b.) one or both of the samples were at least two times the PQL." Prep data to address this.
  # note: (Practical Quantitation Limit (PQL) = Limit of Quantitation (LOQ))

# specify columns needed for RPD analysis
rpd_cols <- c("monitoring_location_id","activity_start_date","activity_type","characteristic_name","result_analytical_method_id","result_value","result_qualifier","result_detection_limit_type_1","result_detection_limit_value_1")
...
#%>%
 # pivot_wider(names_from = activity_type,values_from = result_value) %>%
  #clean_names() %>%
  #select(-field_msr_obs, -quality_control_field_replicate_msr_obs)
...
z <- left_join(field_dup_dat_2021,above_loq) %>%
  ...
  filter(loq_2x != )
    ...
    mutate(rpd_pct = ((field_msr_obs - quality_control_field_replicate_msr_obs) / 
                      ((field_msr_obs + quality_control_field_replicate_msr_obs)/2) ))
  
  # replace non-detects with zero for Relative Percent Difference (RPD) calculations
  replace(is.na(.), 0) %>%

 %>%
  
  
  mutate_at(vars(rpd_pct), ~replace(., is.nan(.), 0))
  
  # replace "Inf" in RPD results with "100% difference" values
  mutate(rpd_pct = case_when(
    is.infinite(rpd_pct), "100"
  ))

# match against standards from QAPP; make cases for where excptions should be addressed (eg for FC; count 1 vs 2 = 50% rpd)
