...
# The code will be executed here rather than in individual chunks above each individual question, because some of the questions are not sequentially independent. (That is to say, values from multiple questions are needed to answer single questions, sometimes from prior and sometimes from after a particular question. For example in question #4, we want to know how many values we excluded from the whole original dataset, which requires decisions based on questions following question #4).
...
## 4.1: Completeness Measure B: "the planned number of samples divided by the useable samples with a goal of 60%."

### calculate values needed for completeness
# create count table of collected results in original aqwms file. flag those results that are not submitted to ADEC. compare two sets of values to get completeness measure A.
...
total_samples_collected_2021 <- read.csv('other/output/aqwms_formatted_results/2021_kwf_baseline_results_aqwms.csv') %>%
  clean_names() %>%
  group_by(result_analytical_method_id) %>%
  count() %>%
  rename(actual_result = n)

### number of samples finalized and submitted to DEC
...
## prepare and summarise table samples collected and tests performed
  ...
  mutate(analysis = case_when(
    analysis == "SM4500-NO3E - Nitrogen (Nitrate+Nitrite)" ~ "SM4500-NO3E",
  ...
  mutate(expected_results = case_when(
    analysis == "SM4500-NO3E - Nitrogen (Nitrate+Nitrite)" ~ 1,
    analysis == "SM4500-PE - Total Phosphorus" ~ 1,
    analysis == "200.7 - Total Metals" ~ 3, # i think27 in 
    analysis == "200.8 - Dissolved Metals" ~ 6, # custom short list; though lab accidentally ran full list of 27 analytes in 2021
    analysis == "1 liter Total Suspended Solids" ~ 1,
    analysis == "Fecal Coliform" ~ 1,
    analysis == "624 - BTEX" ~ 6)) %>%
  group_by(analysis,expected_results) %>%
  summarise(expected_ct = sum(expected_results)) %>%
  select(-expected_results) %>%
  rename(expected_result = expected_ct)

# join planned vs expected tables
...
# i think this (above) is fixed now
# planned vs actual still not quite matching up. cause tbd.
...
# used this code to disocovere thru trial and error that ALS ran lab dups, different than field dups
