...
1.  Did the project follow the QAPP? ***Yes***. Were there any deviations from the sampling plan? ***Refer to Individual Answers Below***
...
3.  Are site names, dates, and times correct and as expected? ***Yes*** Notes: In 2021 Kenai Watershed Forum used pre-printed labels on all sample bottles, reducing opportunity for field and lab transcription errors. Remaining site name transcription errors were corrected in post-season data review.
...
# note 3/22/22 --- we might need to execute/hide all the code needed to populate these answers PRIOR TO their inline text. Because, values from multiple answers are needed to answer single questions and are not sequentially independent. for example question #4, we want to know how many values we excluded from the whole original dataset, which requires decisions based on questions following question #4.

# completeness measures


## measure A: create count table of collected results in original aqwms file. flag those results that are not submitted to ADEC. compare two sets of values to get completeness measure A.
...
## measure B: use table for kit request to get total number of planned samples. compare this table to final set of values that hold up against qa/qc process
```
...
5.  Were field duplicates, blanks, and/or other QC samples collected as planned? ***Yes***

```{r, echo = F}
# How many field blanks, field duplicates, and other QC samples were collected?

## plan: modify kit request to distinguish sample vs. above types. Create df of planned values (impr), join with actual sample summary value derived from aqwms pre-review submission
```
