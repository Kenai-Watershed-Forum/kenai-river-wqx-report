...
sgs21 <- bind_rows(spring_batch_sgs21,summer_batch_sgs21) %>%
  clean_names() %>%
  remove_empty() %>%
  # retain only field sample results
...
# remove from "sample" names the text containing the suffixes Diss/Dis (Dissolved metals sample) since we only want a list of unique sites. (Solution for this step found at https://stackoverflow.com/questions/29271549/replace-all-occurrences-of-a-string-in-a-data-frame)
dat %<>%  
...
3/1/22 -- all we are missing for 2021 is the EDD file for summer ALS data. Integrate in pipeline when available. Good to proceed with final formatting!

Need to figure out how to segregate and designate BTEX Trip Blanks !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (see 'activity type')

working here to un-hashtag one by one
...
z <- dat %>%
  ...
  mutate(
   ...
   # potentially a complex process here 3/2/2022
    ...
    `Result Detection Limit Unit` = units,
