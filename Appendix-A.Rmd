...
als21 <- spring_als21 %>%
  ...
  rename(collect_date = date_collected,
...
# Performed manually by B Meyer February 2022.
...
# segregate laboratory QA/QC data from field data. These data will be evaluated at a late step. See excel file "other/input/AQWMS/sample_type_abbreviations_manual_edit.xlsx" for sample_type naming schema
...
# remaining issue: the ALS Electronic Data Deliverables do not include sample collection time, only date. But - we have the sample collection times populated for all the SGS data. The collection time is identical for both since they're from the same sample.
...
# segregate lab results from field results
swwtp_spring21_qaqc_dat <- swwtp_spring21 %>%
  filter(sample_type %in% c("MB","LCS"))
write.csv(swwtp_spring21_qaqc_dat, "other/output/lab_qaqc_data/2021_lab_qaqc_data/swwtp_spring21_qaqc_dat.csv")
...
WORKING HERE 3/4/22 -- SEGREGATING LAB QAQC FROM FIELD RESULTS. SAVE TO SEPERATE CSVS AND SUMMARISE/ANALYSE LATER
...
############### Miscellaneous Steps for Overall Dataframe #################
...
Still missing 2021 EDD file for summer ALS data. Integrate in pipeline when available.

Need to figure out how to segregate and designate BTEX Trip Blanks !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (see 'activity type')

various types of blanks all named "trip blanks" 

need to figure out statistical base code

find and fix appearance of  "2021-08-27" date
...
dat %<>%
  ...
  mutate(
    ...
    `Statistical Base Code` = "", # may need to ask
...
... we have retained only field data here. see section "DEC Data Evaluation Checklist" for evaluation of laboratory QA/QC data.
