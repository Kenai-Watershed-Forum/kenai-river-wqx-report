...
# Original SGS engage downloads did not contain lab batch info (grey boxes in PDF doc). Acquired upon request in Feb 2022 upon discovery that they existed. 
# We need to join batch info to overall results
# for future years data prep, just use full batch (or "EDD", Electronic Data Delivery) files directly instead of the summary downloads
...
```{r}
# split a.) lab sample run & b.) collect time and date in prep for future join
sgs21 %<>%
  transform(run_date_time = mdy_hm(run_date_time),
            collect_date = mdy_hm(collect_date)) %>%
  mutate(run_date = date(run_date_time),
         run_time = as_hms(run_date_time),
         collect_time = as_hms(collect_date),
         collect_date = date(collect_date)) %>%
  transform(run_time = as.character(run_time))
...
als21 <- spring_als21 %>%
  ...
  mutate(matrix = "Water (Surface, Eff., Ground)",
         lab_name = "ALS Environmental, Kelso, WA",
         run_time = "") %>%
  transform(analysis = as.character(analysis),
...
# run time/date in sgs vs run date only in als

# join SGS data with ALS data
sgs21 <- bind_rows(sgs21,als21)  %>%
  # miscellaneous
  select(-run_date_time)
...
sgs21 %<>%
  ...
  mutate(client_sample = str_replace(client_sample, "DUP|Dup", "")) 
...
sgs21 %<>%  
  mutate(client_sample = (str_replace(client_sample, "Diss|Dis|DUP|Dup", ""))) %>%
...
# We need to remove white spaces, apostrophes, and dashes; because join functions such as "left_join" are often uncooperative with these types of strings. We will need to use joins with site names in next steps.
...
# Performed by B Meyer February 2022.
...
# issue: the ALS Electronic Data Deliverables do not include sample collection time, only date. But - we have the sample collection times populated for all the SGS data. The collection time is identical for both since they're from the same sample.
...
WORKING HERE 2/25/22 TO RECITIFY COLLECT TIME ISSUE DESCRIBED
...
## add lab name
  mutate(lab_name = "Taurianen Engineering, Soldotna, AK")
...
taur_summer21 <- left_join(taur_summer21,taur_summer21_sites,by = "sample_location") 



# here, we are unsuccessful for the join by "sample_location" here for two sites (Poachers and Dow Island) for unclear reasons. Attempted various fixes to allow left_join to proceed correctly but unsuccessful. 
