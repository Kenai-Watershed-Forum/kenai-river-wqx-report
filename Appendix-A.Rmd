...
rpd_values <- rpd_check_dat %>%
  ...
  mutate(`rpd_%` = ((field_msr_obs - quality_control_field_replicate_msr_obs) / 
                      ((field_msr_obs + quality_control_field_replicate_msr_obs)/2))*100) %>%
  mutate(`rpd_%` = abs(round(`rpd_%`,2))) %>%
...
# NEXT::: match against standards from QAPP; advocate for where exceptions should be addressed (eg for FC; count 1 vs 2 = 50% rpd)
# 5/6/2022 - IN PROCESS OF SETTING UP TABLE FOR RPD ASSESSMENT STANDARDS
# NEED TO SET UP DATA ENTRY FOR 2013 - 2016 FOR JACK... see analytica request in shared at agency baseline/miscellaneous... create a couple practice examples
...
-   RPD (Relative Percent Difference) goal from QAPP(%): ***Varies by Parameter, See Current QAPP***
...
# Question: can data from 2014 - 2020 entered manually by R McCormick by adapted to the EPA template, or do we need to start from scratch?

# examine this question separately by lab source (SWWTP, SGS, Taurianen)

# more detailed question: can we get all the column names applied to the RM data file as we have from the 2021 read-in

# examine 2021 fecal coliform data column names
